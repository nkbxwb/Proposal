...
\begin{figure}[h!]
  ...
  \caption{Selected histograms of ratio of imputed variables selected}
\label{fig:clhisto}
\end{figure}


%%%%%%%%%% cape_lapat_ratio
Before we move on to the analyses though, let's have a look at the breakdown of the data. In table \ref{table:chartab} we can see how the MI data compares to the available case data, broken down by if the subject had any systemic therapy for the brain mets. Systemic therapy refers to anything that is not directly targeted to the cancer such as chemotherapy or HER2-directed therapies. Systemic therapy does not include treatments such as resection or targeted radiation. This table was computed via the stacked method.
\begin{table}[!ht]
\centering
\adjustbox{max height=\dimexpr\textheight-5.5cm\relax,
           max width=\textwidth}{
...
\end{tabular}}
\caption{Chemotherapeuic ATE, doubly Robust IPTW weights estimate, AC and MI}
\label{chemorobust}
\end{table}
...
\end{tabular}}
\caption{HER2-directed ATE, doubly Robust IPTW weights estimate, AC and MI}
\label{lapatfull}
\end{table}
\end{center}
