...
\item MNAR: Missing not at random: $p(R=0|Y_{obs},Y_{mis},\psi)$ does not simplify, and the missingness depends on data that we have as well as have not collected. For example if the blood testing machine breaks when the protein level is either too high or too low.
\end{itemize}
